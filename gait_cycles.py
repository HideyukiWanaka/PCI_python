...
def identify_gait_cycles(sync_gyro_df, sampling_rate_hz,
                         swing_threshold=200, swing_duration_threshold_ms=40, # swing_thresholdは調整が必要かも
                         filter_cutoff=15.0,
    ...
    if sync_gyro_df is None or sync_gyro_df.empty: print("エラー:入力データ空"); return None
    results = []; dt = 1.0 / sampling_rate_hz
    ...
    filtered_signals = {}; time_sec = sync_gyro_df['time_aligned_sec'].values

    for leg_prefix in ['L', 'R']:
        col_name = f'{leg_prefix}_Gyro_Z_aligned'
        if col_name not in sync_gyro_df.columns: filtered_signals[leg_prefix] = None; continue
        signal_raw = sync_gyro_df[col_name].values
        signal_filt = lowpass_filter(signal_raw, cutoff=filter_cutoff, fs=sampling_rate_hz)
        if signal_filt is None: filtered_signals[leg_prefix] = None; continue
        ...
        if not swing_starts: print(f"  {leg_prefix}脚: Swing区間検出不可 (閾値={swing_threshold})"); continue
        ...
        if len(ic_candidates) == 0 or len(fo_candidates) == 0 or len(swing_peaks) == 0: print(f"  {leg_prefix}脚: ピーク/谷検出不可"); continue

        num_cycles = 0
        for j in range(len(swing_starts)):
            current_swing_start = swing_starts[j]; current_swing_end = swing_ends[j] if j < len(swing_ends) else len(signal_filt) -1
            possible_ics = ic_candidates[ic_candidates > current_swing_end];
            if len(possible_ics) == 0: continue; ic_idx = possible_ics[0]
            ...
            fo_search_start_idx = ic_idx + fo_wait_samples_dynamic; fo_search_end_idx = next_swing_start
            ...
            ic_time_val = time_sec[ic_idx] if 0 <= ic_idx < len(time_sec) else np.nan; fo_time_val = time_sec[fo_idx] if 0 <= fo_idx < len(time_sec) else np.nan
            cycle_data = { "Leg": leg_prefix, "Cycle": num_cycles + 1, "IC_Index": ic_idx, "FO_Index": fo_idx, "IC_Time": ic_time_val, "FO_Time": fo_time_val }
            results.append(cycle_data); num_cycles += 1
        # print(f"  {leg_prefix}脚: {num_cycles} 歩行周期同定")

    events_df = pd.DataFrame(results)
    if not events_df.empty: events_df = events_df.sort_values(by=['Leg', 'IC_Index']).reset_index(drop=True)
